...
<body>
    ...
    if (!isset($_SESSION['logged']) or !$_SESSION['logged']) {
        ...
        if ($_GET['opt'] == "req") {
            ...
            $result = $conn->query("SELECT * FROM requests WHERE request_teamID='".$_SESSION['teamID']."' AND verified=1 ORDER BY request_date DESC");
            ...
            echo '
            <h2>Team '.$_SESSION['teamID'].' Requested Parts ' . (!isset($_GET['print']) ? '&nbsp; <a href="history.php?opt=req&print=true">Print</a>' : '') . '</h2>
            <h3>Total: '.$conn->query("SELECT COUNT(*) FROM requests WHERE request_teamID='" . $_SESSION['teamID'] ."' AND verified=1")->fetchColumn().'</h3>
            ...
            $result = $conn->query("SELECT * FROM requests WHERE supply_team_id=".$_SESSION['teamID']." ORDER BY request_date DESC");
            ...
            echo '
            <h2>Requests you have filled' . (!isset($_GET['print']) ? '&nbsp; <a href="history.php?opt=filled&print=true">Print</a>' : '') . '</h2>
            <h3>Total: '.$conn->query("SELECT COUNT(*) FROM requests WHERE supply_team_id='" . $_SESSION['teamID'] . "'")->fetchColumn().'</h3>
            <table class="table table-striped table-hover ">
              <thead>
                <tr>
                  <th>Date</th>
